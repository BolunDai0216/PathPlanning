...
def CreateUnitSphere(r=1):
    phi = np.linspace(0, 2 * np.pi, 256).reshape(
        256, 1
    )  # the angle of the projection in the xy-plane
    theta = np.linspace(0, np.pi, 256).reshape(
        -1, 256
...
class BIT_star:
        ...
        while True:
            ...
            if len(self.QE) == 0 and len(self.QV) == 0:
                ...
                if self.done:
                    ...
                    self.r = self.radius(
                        len(self.V) + len(self.Xsamples)
                    )  # radius determined with the sample size and dimension of conf space
            while self.BestQueueValue(self.QV, mode="QV") <= self.BestQueueValue(
                self.QE, mode="QE"
            ):
                self.ExpandVertex(self.BestInQueue(self.QV, mode="QV"))
            (vm, xm) = self.BestInQueue(self.QE, mode="QE")
            self.QE.remove((vm, xm))
            if self.g_T(vm) + self.c_hat(vm, xm) + self.h_hat(xm) < self.g_T(
                self.xgoal
            ):
                cost = self.c(vm, xm)
                if self.g_hat(vm) + cost + self.h_hat(xm) < self.g_T(self.xgoal):
                    if self.g_T(vm) + cost < self.g_T(xm):
                        if xm in self.V:
                            self.E.difference_update(
                                {(v, x) for (v, x) in self.E if x == xm}
                            )
                        else:
                        ...
                        self.QE.difference_update(
                            {
                                (v, x)
                                for (v, x) in self.QE
                                if x == xm
                                and (self.g_T(v) + self.c_hat(v, xm)) >= self.g_T(xm)
                            }
                        )
                    ...
                    x[
                        np.array(
                            [
                                ...
                                and isinbound(self.env.boundary, state)
                                for state in x
                            ]
                        )
                    ],
        ...
        self.QE.update(
            {
                (v, x)
                for x in Xnear
                if self.g_hat(v) + self.c_hat(v, x) + self.h_hat(x)
                < self.g_T(self.xgoal)
            }
        )
            ...
            self.QE.update(
                {
                    (v, w)
                    for w in Vnear
                    if ((v, w) not in self.E)
                    and (
                        self.g_hat(v) + self.c_hat(v, w) + self.h_hat(w)
                        < self.g_T(self.xgoal)
                    )
    ...
    def Prune(self, c):
        ...
        self.E.difference_update(
            {(v, w) for (v, w) in self.E if (self.f_hat(v) > c) or (self.f_hat(w) > c)}
        )
    ...
    def BestInQueue(self, inputset, mode):
        ...
        if mode == "QE":
            V = {
                state: self.g_T(state[0])
                + self.c_hat(state[0], state[1])
                + self.h_hat(state[1])
                for state in self.QE
            }
        if len(V) == 0:
            print(mode + "empty")
            return None
    ...
    def visualization(self):
        if self.ind % 20 == 0:
            ...
            ax.view_init(elev=90.0, azim=60.0)
