...
class FMT_star:
    def __init__(self, radius=1, n=1000):
        ...
        self.x0, self.xt = tuple(self.env.start), tuple(
            self.env.goal
        )  # used for sample free
    ...
    def initNodeSets(self):
        ...
        Vopen_queue.put(
            self.xinit, c[self.xinit]
        )  # priority organized as the cost to come
    ...
    def FMTrun(self):
        ...
        while z != self.xgoal:
            ...
            for x in Xnear:
                ...
                ymin = Ynear[
                    np.argmin([self.c[y] + self.Cost(y, x) for y in Ynear])
