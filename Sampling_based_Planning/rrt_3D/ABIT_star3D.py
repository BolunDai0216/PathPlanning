...
class ABIT_star:
        ...
        while True:
            if self.is_search_marked_finished():
                ...
                if (xp, xc) in E:
                    ...
                    self.g_T(xp) + self.c_hat(xi, xj) + self.h_hat(xc)
                ) <= self.g_T(self.xgoal):
                    if self.g_T(xp) + self.c_hat(xp, xc) < self.g_T(xc):
                        if self.g_T(xp) + self.c(xp, xc) + self.h_hat(xc) < self.g_T(
                            self.xgoal
                        ):
                            if self.g_T(xp) + self.c(xp, xc) < self.g_T(xc):
                                ...
                                if xc in Vclosed:
                                    ...
                                    Q.update(
                                        self.expand({xc}, T, Xunconnected, self.r(q))
                                    )
                                    Vclosed.add(xc)
                else:
    ...
    def prune(self, T, Xunconnected, xgoal):
        V, E = T
        Xunconnected.difference_update(
            {x for x in Xunconnected if self.f_hat(x) >= self.g_T(xgoal)}
        )
    ...
    def r(self, q):
        return self.eta * (
            2
            * (1 + 1 / self.n)
            * (self.Lambda(self.Xf_hat) / self.Zeta)
            * (np.log(q) / q)
        ) ** (1 / self.n)
