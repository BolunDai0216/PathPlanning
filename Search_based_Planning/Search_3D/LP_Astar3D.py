...
class Lifelong_Astar(object):
    def __init__(self, resolution=1):
        ...
        self.g = g_Space(self)
        self.start, self.goal = getNearest(self.g, self.env.start), getNearest(
            self.g, self.env.goal
        )
    ...
    def ComputePath(self):
        ...
        while (
            self.key(self.xt) > self.OPEN.top_key()
            or self.rhs[self.xt] != self.g[self.xt]
        ):
            xi = self.OPEN.get()
